    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-grow pt-32 pb-20 px-4">
        <div className="max-w-[1600px] mx-auto">
          ...
          <Link href="/articles" className="inline-flex items-center gap-2 text-sm text-primary hover:underline mb-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
            <div>
              <h1 className="text-3xl font-bold text-text-primary mb-2">Redactar Artículo</h1>
              <p className="text-text-secondary text-sm">Tu artículo quedará pendiente de aprobación del administrador antes de ser público.</p>
            </div>
            ...
            <div className="flex gap-3 items-center">
              ...
              <button onClick={handlePublish} disabled={isSubmitting || !title || !abstract} className="btn-primary shrink-0">
                <Send className="w-4 h-4" /> {isSubmitting ? 'Enviando...' : 'Publicar Investigación'}
              </button>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-primary block">Título del artículo</label>
              <input
                type="text"
                value={title}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-primary block">Área de investigación</label>
              <select
                value={researchArea}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-primary block">Resumen (Abstract)</label>
              <input
                type="text"
                value={abstract}
          ...
          <div className={`grid gap-6 ${view === 'split' ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
            ...
            {(view === 'edit' || view === 'split') && (
              ...
              >
                <div className="p-3 border-b border-border bg-surface-muted">
                  <span className="text-xs font-medium text-text-muted">Código fuente (Markdown)</span>
                </div>
                <textarea
                  value={content}
            ...
            {(view === 'preview' || view === 'split') && (
              ...
              >
                <div className="p-3 border-b border-border bg-surface-muted">
                  <span className="text-xs font-medium text-text-muted">Vista previa renderizada</span>
                </div>
