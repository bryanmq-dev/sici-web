...
export default function DashboardClient({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "quests" | "badges" | "eventos" | "nivel"
  >("overview");
                ...
                <>
                  <div className="h-1.5 bg-surface-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, ((user.isiPoints - level.minPoints) / (nextLevel.minPoints - level.minPoints)) * 100)}%`,
                      }}
                    />
                  </div>
                  <div className="text-[10px] text-text-muted mt-1">
                    {nextLevel.minPoints - user.isiPoints} pts para{" "}
                    {nextLevel.name}
                  </div>
                </>
              )}
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-text-muted mb-4">
                    Siempre activas — tu progreso avanza automáticamente según
                    lo que hagas en la plataforma.
                  </p>
                </div>
                        ...
                        <div className="mt-3">
                          <div className="h-1.5 bg-surface-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${userQuest?.progress ?? 0}%` }}
                            />
                          </div>
                          <div className="text-xs text-text-muted mt-1">
                            {userQuest?.progress ?? 0}% completado
                          </div>
                        </div>
                      </div>
                    );
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {userBadges.map((userBadge) => {
                    const Icon =
                      iconMap[userBadge.badgeIcon || "Award"] || Award;
                    const rarity = userBadge.badgeRarity || "COMMON";

                    return (
                      <div
                        key={userBadge.id}
                        className="card p-4 text-center relative"
                      >
                        {userBadge.count > 1 && (
                          <span className="absolute top-2 right-2 badge badge-primary text-[10px]">
                            x{userBadge.count}
                          </span>
                        )}
                        <div
                          className={`w-16 h-16 mx-auto mb-3 flex items-center justify-center rounded-xl ${
                            rarity === "LEGENDARY"
                              ? "bg-amber-500/10"
                              : rarity === "EPIC"
                                ? "bg-purple-500/10"
                        ...
                        <span className="badge badge-secondary text-[10px] mt-2">
                          {rarity}
                        </span>
                      </div>
                    );
            ...
            {activeTab === "eventos" && (
              ...
              >
                ...
                <p className="text-xs text-text-muted mb-4">
                  Misiones especiales ligadas a eventos, evaluadas por el admin.
                </p>
                <div className="space-y-3">
                  {eventParticipations.map((ep) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-text-muted">
                          {ep.intent === "collaborate" ? "Colaborar" : "Apoyo"}
                          {ep.scoreDescription
                            ? ` — ${ep.scoreDescription}`
                            : ""}
                        </div>
                  ...
                  {eventParticipations.length === 0 && (
                    <p className="text-sm text-text-muted text-center py-8">
                      No te has anotado a ningún evento todavía
                    </p>
                  )}
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="space-y-3">
                  {LEVELS.map((l, i) => (
                    <div
                      key={l.name}
                      className={`card p-4 flex items-center justify-between ${l.name === level.name ? "border-primary" : ""}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${l.name === level.name ? "bg-primary text-white" : "bg-surface-muted text-text-muted"}`}
                        >
                          {i + 1}
                        </div>
                        <div className="text-sm font-medium text-text-primary">
