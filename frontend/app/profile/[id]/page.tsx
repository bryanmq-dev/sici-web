...
import { 
  ...
  ExternalLink, Award, Book, Code, ArrowLeft, ArrowUpRight, 
...
export default function UserProfilePage() {
  ...
  // Mock check - in a real app this would compare auth.user.id with params.id
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-grow flex items-center justify-center p-4">
          <div className="text-center">
            <h1 className="text-4xl font-display font-bold mb-4 uppercase tracking-tighter">Usuario no encontrado</h1>
            <Link href="/ranking" className="hud-button">Volver al Ranking</Link>
          </div>
        </main>
        <Footer />
      </div>
      ...
      <main className="flex-grow pt-32 pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-12">
            <Link href="/ranking" className="inline-flex items-center gap-2 text-[10px] font-mono text-primary hover:underline uppercase tracking-widest">
              <ArrowLeft className="w-3 h-3" /> VOLVER_AL_RANKING
            </Link>
            
            {isOwnProfile && (
              <button 
                onClick={() => setIsEditModalOpen(true)}
                className="flex items-center gap-2 text-[10px] font-mono text-primary border border-primary/20 px-4 py-2 bg-primary/5 hover:bg-primary/10 transition-all uppercase tracking-widest"
              >
                <Edit3 className="w-3 h-3" /> Editar_Perfil
              </button>
            )}
          </div>
